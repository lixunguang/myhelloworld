...
#ifdef CUSTOMIZE_V4_WORK
    connect(ribbonBar(), SIGNAL(showRibbonContextMenu(QMenu*, bool&)), this, SLOT(showRibbonContextMenu(QMenu*, bool&)));
#endif // CUSTOMIZE_V4_WORK
...
void MainWindow::createAction()
{
    ...
    m_actionSaveFile = new QAction(iconSave, tr("&Save"), this);
...
void MainWindow::createRibbon()
{
    ...
    {
        pageReview->setContextColor(RibbonPage::ContextColorPurple);
    }
...
void MainWindow::options(QAction* action)
{
    ...
    QPixmap px(":/res/background.png");
    if (action->objectName() == "Office2013Dark")
...
#ifdef CUSTOMIZE_V4_WORK
void MainWindow::showRibbonContextMenu(QMenu* menu, bool& showMenu)
{
    Q_UNUSED(menu)
    showMenu = true;
}
...
{
    RibbonCustomizationManager* customizationManager = ribbonBar()->customizeManager();

    customizationManager->setCommandName(m_actionFileNew, "_qtn_action_Save");
    customizationManager->addActionToCategory(m_actionFileNew, categoryPopularCommands);

    customizationManager->setCommandName(m_actionOpenFile, "_qtn_action_Open_File");
    customizationManager->addActionToCategory(m_actionOpenFile, categoryPopularCommands);

    customizationManager->setCommandName(m_actionSaveFile, "_qtn_action_Save_File");
    customizationManager->addActionToCategory(m_actionSaveFile, categoryPopularCommands);

    customizationManager->setCommandName(m_actionPrint, "_qtn_action_Print");
    customizationManager->addActionToCategory(m_actionPrint, categoryPopularCommands);

    customizationManager->setCommandName(m_actionPrintPreview, "_qtn_action_Print_Preview");
    customizationManager->addActionToCategory(m_actionPrintPreview, categoryPopularCommands);

    customizationManager->addCategorySeparator();
}
#endif // CUSTOMIZE_V4_WORK
...
void MainWindow::readSettings()
{
    ...
    Qtitan::OfficeStyle::Theme theme = (Qtitan::OfficeStyle::Theme)settings.value("theme", 
        (int)Qtitan::OfficeStyle::Office2013White).toInt();
...
#ifdef CUSTOMIZE_V4_WORK
    QByteArray state = settings.value("stateRibbon").toByteArray();
    ribbonBar()->customizeManager()->restoreState(state);
#endif // CUSTOMIZE_V4_WORK
...
#ifdef CUSTOMIZE_V4_WORK
    QByteArray state = ribbonBar()->customizeManager()->saveState();
    settings.setValue("stateRibbon", state);
#endif // CUSTOMIZE_V4_WORK
